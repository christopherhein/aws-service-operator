...
func Subscribe(config *config.Config, ctx context.Context) {
	...
	process(config, svc, config.QueueManager, ctx)
}
...
// SetQueuePolicy will update the queue to reflect the proper policy
func SetQueuePolicy(config *config.Config) error {
	sqsSvc := sqs.New(config.AWSSession)
	topicARNs := config.QueueManger.Keys()
...
func process(config *config.Config, svc *sqs.SQS, manager *queuemanager.QueueManager, ctx context.Context) {
	logger := config.Logger
	for {
		...
		for _, item := range result.Messages {
			mess, err := message.New(item)
			if err != nil {
				logger.WithError(err).Error("error parsing message body")
			}

			h, ok := manager.Get(mess.TopicARN())
			if !ok {
				logger.Errorf("error missing handler function for topic %s", mess.TopicARN())
			}

			err = h.HandleMessage(config, mess)
			if err != nil {
				logger.WithError(err).Error("error processing message")
			}
			logger.Debugf("stackID %v updated status to %v", mess.StackID(), mess.ResourceStatus())
